...
app.get('/api/test-grades', async (req, res) => {
  try {
    ...
    // Check if request is for all schools
    if (school === 'all') {
      console.log('Getting grades distribution for all schools');
      const data = await getGradesDistributionForAllSchools();
      return res.json({
        school: 'All Schools',
        data,
        debug: {
...
// Add a helper function to get grades distribution for all schools
async function getGradesDistributionForAllSchools(): Promise<PieChartData[]> {
  try {
    const query = `
      WITH RECURSIVE 
      all_categories(category) AS (
    ...
    if (total === 0) {
      console.log('No grades data found for all schools');
      return [
        { label: 'No hay datos', value: 1, color: '#CCCCCC' }
      ];
    }
    ...
    const chartData = result.rows.map(row => {
      ...
      // Normalize the grade format to handle both ° and º symbols
      const normalizedGrade = row.category as string;
      console.log('Normalized grade:', normalizedGrade);
...
async function getGradosEstudiantesDistribution(school: string): Promise<PieChartData[]> {
  try {
    // Implementation has been moved to line 1926 with the updated return type BarChartData[]
    throw new Error('This implementation has been replaced');
  } catch (error) {
...
async function getGradesDistributionForEstudiantes(school: string): Promise<PieChartData[]> {
  try {
    // If requesting all schools data, modify the query to ignore the school filter
    const isAllSchools = school === 'all';
    ...
    const query = `
      SELECT 
        ...
        COUNT(*) as count
      FROM estudiantes_form_submissions
      ${!isAllSchools ? 'WHERE institucion_educativa = $1' : ''}
    ...
    const result = await pool.query(isAllSchools ? query : query, isAllSchools ? [] : [school]);
    console.log('Raw grades distribution result:', result.rows);
...
async function getScheduleDistributionForEstudiantes(school: string): Promise<PieChartData[]> {
  try {
    // Implementation has been moved to line 1883 with the updated return type BarChartData[]
    throw new Error('This implementation has been replaced');
  } catch (error) {
    console.error('Error in getScheduleDistributionForEstudiantes:', error);
    return [];
  }
} 
...
app.get('/api/estudiantes-grades', async (req, res) => {
  try {
    ...
    // Use the school parameter as is, allowing 'all' as a value
    const data = await getGradesDistributionForEstudiantes(school);
...
// Add new endpoint for school names dropdown
app.get('/api/schools', async (req, res) => {
  try {
    ...
    console.log('Executing schools dropdown query');
    const schoolsResult = await pool.query(schoolsQuery);
...
// Add functions for years and feedback distribution
async function getYearsDistribution(school?: string): Promise<BarChartData[]> {
  try {
    console.log('Fetching years distribution for school:', school || 'All schools');
    ...
    const result = await pool.query(query, school ? [school] : []);
...
async function getFeedbackDistribution(school?: string): Promise<BarChartData[]> {
  try {
    console.log('Fetching feedback distribution for school:', school || 'All schools');
    ...
    const checkQuery = `
      ...
      LIMIT 10;
    `;
    const checkResult = await pool.query(checkQuery, school ? [school] : []);
    ...
    const { rows } = await pool.query(query, school ? [school] : []);
...
// Add endpoint for years distribution
app.get('/api/years-distribution', async (req, res) => {
  try {
    const school = req.query.school as string;
    if (!school) {
      console.log('Missing school parameter');
    ...
    console.log('Getting years distribution for school:', school);
    // If 'all' is passed, don't filter by school
    const schoolParam = school === 'all' ? undefined : decodeURIComponent(school);
    const data = await getYearsDistribution(schoolParam);
    console.log('Years distribution data:', JSON.stringify(data, null, 2));
    
    res.json({
      school: school === 'all' ? 'All Schools' : decodeURIComponent(school),
      data
    });
  } catch (error) {
    console.error('Error fetching years distribution:', error);
...
// Add endpoint for estudiantes years distribution
app.get('/api/estudiantes-years-distribution', async (req, res) => {
  try {
    const school = req.query.school as string;
    if (!school) {
      console.log('Missing school parameter');
    ...
    console.log('Getting estudiantes years distribution for school:', school);
    // If 'all' is passed, don't filter by school
    const schoolParam = school === 'all' ? undefined : decodeURIComponent(school);
    const data = await getYearsDistributionForEstudiantes(schoolParam);
    console.log('Estudiantes years distribution data:', JSON.stringify(data, null, 2));
...
// Add endpoint for feedback distribution
app.get('/api/feedback-distribution', async (req, res) => {
  try {
    const school = req.query.school as string;
    if (!school) {
      console.log('Missing school parameter');
    ...
    console.log('Getting feedback distribution for school:', school);
    // If 'all' is passed, don't filter by school
    const schoolParam = school === 'all' ? undefined : decodeURIComponent(school);
    const data = await getFeedbackDistribution(schoolParam);
    console.log('Feedback distribution data:', JSON.stringify(data, null, 2));
...
// Add endpoint for estudiantes schedule distribution - replace all duplicate implementations
app.get('/api/schedule-distribution-estudiantes', async (req, res) => {
  try {
    const school = req.query.school as string;
    if (!school) {
      console.log('Missing school parameter');
    ...
    console.log('Getting schedule distribution for estudiantes, school:', school);
    // If 'all' is passed, don't filter by school
    const schoolParam = school === 'all' ? undefined : decodeURIComponent(school);
    
    // Modify the query to not filter by school if schoolParam is undefined
    const query = `
      SELECT 
        jornada as schedule,
        COUNT(*) as count
      FROM estudiantes_form_submissions
      ${schoolParam ? 'WHERE institucion_educativa = $1' : ''}
      GROUP BY jornada
      ORDER BY count DESC;
    `;

    console.log('Executing query:', query.replace(/\n\s*/g, ' '));
    
    const result = await pool.query(
      query, 
      schoolParam ? [schoolParam] : []
    );
    console.log('Query result for estudiantes schedule:', JSON.stringify(result.rows, null, 2));
    
    // Normalize schedule names and consolidate counts
    ...
    // Define the standard categories and their colors
    const standardCategories = [
      { name: 'Mañana', color: '#4e73df' },
      { name: 'Tarde', color: '#1cc88a' },
      { name: 'Noche', color: '#f6c23e' },
      { name: 'Única', color: '#7030A0' }
    ];
    ...
    result.rows.forEach(row => {
      const normalizedSchedule = scheduleMapping[row.schedule] || 'Única'; // Default to 'Única' if unknown
      consolidatedCounts[normalizedSchedule] += parseInt(row.count);
    });
    
    // Convert to the expected format
    const scheduleData = standardCategories.map(category => ({
      label: category.name,
      value: consolidatedCounts[category.name],
      color: category.color
    })).filter(item => item.value > 0); // Only include categories that have data
...
// Add endpoint for acudientes grade distribution - replace all duplicate implementations
app.get('/api/grados-acudientes-distribution', async (req, res) => {
  try {
    const school = req.query.school as string;
    if (!school) {
      console.log('Missing school parameter');
    ...
    console.log('Getting grados for acudientes, school:', school);
    // If 'all' is passed, don't filter by school
    const schoolParam = school === 'all' ? undefined : decodeURIComponent(school);
    ...
    // Map the data
    const gradesData = result.rows.map((row, index) => ({
      label: row.grade || 'Sin especificar',
      value: parseInt(row.count),
...
// around line 1244
async function getScheduleDistributionForEstudiantesOld(school: string): Promise<PieChartData[]> {
  try {
    // Implementation has been moved to line 1883 with the updated return type BarChartData[]
    throw new Error('This implementation has been replaced');
  } catch (error) {
    console.error('Error in getScheduleDistributionForEstudiantesOld:', error);
    return [];
  }
}

// Add an endpoint for docentes schedule
app.get('/api/schedule-distribution-docentes', async (req, res) => {
  try {
    const school = req.query.school as string;
    if (!school) {
      console.log('Missing school parameter');
    ...
    console.log('Getting schedule distribution for docentes, school:', school);
    // If 'all' is passed, don't filter by school
    const schoolParam = school === 'all' ? undefined : decodeURIComponent(school);
    ...
    console.log('Query result for docentes schedule:', JSON.stringify(result.rows, null, 2));
...
// Function to get years distribution for estudiantes
async function getYearsDistributionForEstudiantes(school?: string): Promise<BarChartData[]> {
  try {
    console.log('Fetching years distribution for estudiantes, school:', school || 'All schools');
    ...
    console.log('Executing query:', query.replace(/\s+/g, ' '));
    const result = await pool.query(query, school ? [school] : []);
    console.log('Years distribution raw data for estudiantes:', result.rows);
    ...
    // Transform the data with display labels
    const chartData = result.rows.map(row => {
      console.log('Processing row for estudiantes:', row);
    ...
    // If no data, return default structure with display labels
    if (chartData.length === 0) {
      console.log('No data found for estudiantes, returning default structure');
      return [
        { label: 'Menos de 1', value: 0, color: '#4472C4' },
        { label: '1 año', value: 0, color: '#ED7D31' },
        { label: '2 años', value: 0, color: '#A5A5A5' },
        ...
        { label: '4 años', value: 0, color: '#5B9BD5' },
        { label: '5 años', value: 0, color: '#70AD47' },
        { label: 'Más de 5', value: 0, color: '#7030A0' }
      ];
    }
...
// Add endpoint for school rankings (Palmares)
app.get('/api/schools-ranking', async (req, res) => {
  try {
    console.log('Calculating school rankings');

    // First, get all schools
    const schoolsQuery = `
      SELECT DISTINCT institucion_educativa
      FROM docentes_form_submissions
      WHERE institucion_educativa IS NOT NULL AND institucion_educativa != ''
      ORDER BY institucion_educativa;
    `;
    const schoolsResult = await pool.query(schoolsQuery);
    const schools = schoolsResult.rows.map(row => row.institucion_educativa);
    
    console.log(`Found ${schools.length} schools to rank`);
    ...
    // Calculate weighted scores for each school
    const rankings = await Promise.all(schools.map(async (school) => {
      try {
        // Query all ratings from all tables for this school
        const query = `
          WITH all_ratings AS (
            -- Docentes ratings - comunicacion
            SELECT
              key as question_key,
              LOWER(TRIM(value)) as rating_value,
              COUNT(*) as count
            FROM docentes_form_submissions,
              jsonb_each_text(comunicacion) as com(key, value)
            WHERE institucion_educativa = $1
              AND LOWER(TRIM(value)) IN ('siempre', 'casi siempre', 'a veces', 'casi nunca', 'nunca')
            GROUP BY key, LOWER(TRIM(value))
            
            UNION ALL
            
            -- Docentes ratings - practicas_pedagogicas
            SELECT
              key as question_key,
              LOWER(TRIM(value)) as rating_value,
              COUNT(*) as count
            FROM docentes_form_submissions,
              jsonb_each_text(practicas_pedagogicas) as pp(key, value)
            WHERE institucion_educativa = $1
              AND LOWER(TRIM(value)) IN ('siempre', 'casi siempre', 'a veces', 'casi nunca', 'nunca')
            GROUP BY key, LOWER(TRIM(value))
            ...
            -- Docentes ratings - convivencia
            SELECT
              key as question_key,
              LOWER(TRIM(value)) as rating_value,
              COUNT(*) as count
            FROM docentes_form_submissions,
              jsonb_each_text(convivencia) as conv(key, value)
            WHERE institucion_educativa = $1
              AND LOWER(TRIM(value)) IN ('siempre', 'casi siempre', 'a veces', 'casi nunca', 'nunca')
            GROUP BY key, LOWER(TRIM(value))
            
            -- Estudiantes ratings - comunicacion
            UNION ALL
            ...
            -- Estudiantes ratings - practicas_pedagogicas
            SELECT
              key as question_key,
              LOWER(TRIM(value)) as rating_value,
              COUNT(*) as count
            ...
            -- Estudiantes ratings - convivencia
            SELECT
              key as question_key,
              LOWER(TRIM(value)) as rating_value,
              COUNT(*) as count
            ...
            -- Acudientes ratings - comunicacion
            UNION ALL
            ...
            -- Acudientes ratings - practicas_pedagogicas
            SELECT
              key as question_key,
              LOWER(TRIM(value)) as rating_value,
              COUNT(*) as count
            ...
            -- Acudientes ratings - convivencia
            SELECT
              key as question_key,
              LOWER(TRIM(value)) as rating_value,
              COUNT(*) as count
        ...
        console.log(`Executing query for school: ${school}`);
        const result = await pool.query(query, [school]);
        ...
        // siempre = 4 points, casi siempre = 3, a veces = 2, casi nunca = 1, nunca = 0
        let weightedScore = 0;
        let ratingCounts = {
          siempre: 0,
          casi_siempre: 0,
        ...
        result.rows.forEach(row => {
          const count = parseInt(row.count);
          const rating = row.rating_value.toLowerCase().trim();
          
          if (rating === 'siempre') {
            weightedScore += count * 4;
            ratingCounts.siempre = count;
          } else if (rating === 'casi siempre') {
            weightedScore += count * 3;
            ratingCounts.casi_siempre = count;
          } else if (rating === 'a veces') {
            weightedScore += count * 2;
            ratingCounts.a_veces = count;
          } else if (rating === 'casi nunca') {
            weightedScore += count * 1;
            ratingCounts.casi_nunca = count;
          } else if (rating === 'nunca') {
            weightedScore += count * 0;
            ratingCounts.nunca = count;
          }
        });
        
        // Calculate average score (out of 4)
        const averageScore = total > 0 ? weightedScore / total : 0;
        
        return {
          school,
          totalResponses: total,
          averageScore: parseFloat(averageScore.toFixed(2)),
          percentageScore: parseFloat((averageScore / 4 * 100).toFixed(1)),
    ...
    // Add rank position
    const rankedSchools = rankings.map((school, index) => ({
      rank: index + 1,
      ...school
    }));
    
    res.json({
      message: `Successfully ranked ${schools.length} schools`,
      rankings: rankedSchools
    });
  } catch (error) {
    console.error('Error calculating school rankings:', error);
    res.status(500).json({ 
      error: 'Internal server error',
      details: error instanceof Error ? error.message : 'Unknown error'
    });
