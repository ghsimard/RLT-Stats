  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Use 'all' as a parameter if that's the school value
        const schoolParam = school === 'all' ? 'all' : encodeURIComponent(school);
        
        const response = await fetch(
          `${config.api.baseUrl}/api/${endpoint}?school=${schoolParam}`
        );
  ...
  return (
    <Box>
      <Typography variant="h6" align="center" gutterBottom>
        {type === 'docentes' && 'Distribución de Grados - Docentes'}
        {type === 'estudiantes' && 'Distribución de Grados - Estudiantes'}
        {type === 'acudientes' && 'Distribución de Grados - Acudientes'}
      </Typography>
      <Box sx={{ 
        maxWidth: 400, 
        margin: '0 auto', 
