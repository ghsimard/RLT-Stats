...
export const generateCoverPage = async (doc: CustomPDFKit, school?: string): Promise<void> => {
  ...
  // Add the cosmoStats logo at the bottom left of the page
  try {
    const cosmoLogoPath = await findImagePath('LogoCosmo.png');
    
    if (cosmoLogoPath) {
      // Define logo dimensions and position
      const cosmoLogoWidth = 150;
      const cosmoLogoHeight = 50;
      const bottomMargin = 40;
      const leftMargin = 40;  // Added left margin
      
      // Position is on the left side, and aligned to bottom with margin
      const cosmoLogoX = leftMargin;  // Changed from centered to left aligned
      const cosmoLogoY = pageHeight - cosmoLogoHeight - bottomMargin;
      ...
      doc.image(
        ...
        cosmoLogoY,
        {
          fit: [cosmoLogoWidth, cosmoLogoHeight]
          // Removed align and valign options that were causing errors
        }
      );
      console.log('CosmoStats logo added successfully at the bottom left');
    } else {
      console.log('CosmoStats logo file not found');
    }
  } catch (error) {
    console.error('Error adding CosmoStats logo to the bottom of cover page:', error);
  }
}; 