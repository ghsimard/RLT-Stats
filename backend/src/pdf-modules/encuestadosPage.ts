...
async function getGradosEstudiantesDistribution(school: string): Promise<PieChartData[]> {
  try {
    ...
    const gradeColors: { [key: string]: string } = {
      'Preescolar': '#6600CC',      // Changed from '#FF9F40' to Purple
      'Primera infancia': '#00CCCC', // Changed from '#FF9F40' to Teal
      '1': '#4472C4',               // Blue
...
export const generateEncuestadosPage = async (doc: CustomPDFKit, school: string): Promise<void> => {
  ...
  // Add ESTUDIANTES title with background - moved higher up
  const estudiantesY = barChartsEndY - 25;  // Changed from -10 to -25 to move up further
  ...
  doc.fontSize(10)
     ...
     .text('¿En qué grado se encuentran los estudiantes que representa?', startX, stackedBarY - 20, {  // Changed from -30 back to -20
       width: doc.page.width - (startX * 2),  // Full page width minus margins
       align: 'left'
     });

  // Draw the stacked bar (moved down to create more space from title)
  const adjustedStackedBarY = stackedBarY + 10;  // Added 10px more space between title and bar
  let currentX = startX;
  ...
  stackedBarData.forEach((item, index) => {
    ...
    // Ensure small segments have a minimum visible width (at least 3px)
    const minWidth = item.value > 0 ? Math.max(3, (item.value / totalStudents) * fullWidth) : 0;
    
    // Draw bar segment
    doc.rect(currentX, adjustedStackedBarY, minWidth, stackedBarHeight)  // Changed y-position to adjustedStackedBarY
       .fillColor(item.color)
       .fill();

    // Add percentage label if segment is wide enough
    if (percentage >= 8) {  // Changed from 10% to 6%
      const percentageTextWidth = doc.widthOfString(`${percentage}%`);  // Get width of text
      const textX = currentX + (minWidth - percentageTextWidth)/2;  // Center text in segment
      const textY = adjustedStackedBarY + (stackedBarHeight/2); // Center text in segment
      
      // Map colors to new scheme (green, blue, red)
      let textColor = item.color;
      if (item.color === '#4472C4' || item.color.toLowerCase() === '#4472c4' || 
          item.color === '#70AD47' || item.color.toLowerCase() === '#70ad47' ||
          item.color === '#2E7D32' || item.color.toLowerCase() === '#2e7d32') {
        textColor = '#009900'; // Soothing green
      } else if (item.color === '#FFC000' || item.color.toLowerCase() === '#ffc000' ||
                 item.color === '#ED7D31' || item.color.toLowerCase() === '#ed7d31' ||
                 item.color === '#D84315' || item.color.toLowerCase() === '#d84315' ||
                 item.color === '#FFFF00' || item.color.toLowerCase() === '#ffff00') {
        textColor = '#4472C4'; // Blue instead of yellow
      } else if (item.color === '#FF0000' || item.color.toLowerCase() === '#ff0000' ||
                 item.color === '#B71C1C' || item.color.toLowerCase() === '#b71c1c') {
        textColor = '#FF0000'; // Standard red
      }
      
      // Use contrasting color based on the segment color
      let contrastColor;
      if (textColor === '#009900') {  // Green
        contrastColor = '#FFFFFF';    // White for contrast
      } else if (textColor === '#4472C4') {  // Blue
        contrastColor = '#FFFFFF';    // White for contrast
      } else if (textColor === '#FF0000') {  // Red
        contrastColor = '#FFFFFF';    // White for contrast
      } else {
        // For other colors, use black or white based on brightness
        const isLight = isLightColor(textColor);
        contrastColor = isLight ? '#000000' : '#FFFFFF';
      }
      
      // Add text shadow for better readability
      doc.fillColor(contrastColor)
         .fontSize(8)
         .text(`${percentage}%`,
           textX,
           textY,
      ...
      // For small segments, draw diagonal line and external label
      const centerX = currentX + minWidth/2;
      ...
      // Determine position (alternating up/down) based on index
      const positionUp = index % 2 === 0;
      
      // Draw diagonal line in the same color as the segment
      doc.save()
         .moveTo(centerX, adjustedStackedBarY + (positionUp ? 0 : stackedBarHeight))  // Changed y-position to adjustedStackedBarY
         .lineTo(centerX - 12, adjustedStackedBarY + (positionUp ? -10 : stackedBarHeight + 10)) // Changed y-position to adjustedStackedBarY
         .strokeColor(textColor) // Same color as the segment
         .lineWidth(0.75)
         .stroke()
         .restore();
      
      // Draw text label centered with diagonal line
      doc.fillColor(textColor)
         .fontSize(7)
         .text(`${percentage}%`, 
               centerX - 25, // Increased left offset from -20 to -25
               adjustedStackedBarY + (positionUp ? -10 : stackedBarHeight + 5), // Changed y-position to adjustedStackedBarY
               { width: 14, align: 'center' }); // Reduced width to center text
    }

    currentX += minWidth; // Remove the segment padding
  });

  // Draw legend below stacked bar
  const legendY = adjustedStackedBarY + stackedBarHeight + 25;  // Changed from stackedBarY to adjustedStackedBarY
  const legendItemWidth = 75;  // Increased width from 60 to 75 to fit percentages
  const legendSpacing = 10;    // Increased from 5 to 10
  
  // Update color mappings to use green, blue, red
  const colorMapping = (color: string) => {
    if (color === '#4472C4' || color.toLowerCase() === '#4472c4' || 
        color === '#70AD47' || color.toLowerCase() === '#70ad47' ||
        color === '#2E7D32' || color.toLowerCase() === '#2e7d32') {
      return '#009900'; // Soothing green
    } else if (color === '#FFC000' || color.toLowerCase() === '#ffc000' ||
               color === '#ED7D31' || color.toLowerCase() === '#ed7d31' ||
               color === '#D84315' || color.toLowerCase() === '#d84315' ||
               color === '#FFFF00' || color.toLowerCase() === '#ffff00') {
      return '#4472C4'; // Blue instead of yellow
    } else if (color === '#FF0000' || color.toLowerCase() === '#ff0000' ||
               color === '#B71C1C' || color.toLowerCase() === '#b71c1c') {
      return '#FF0000'; // Standard red
    }
    return color; // Default case
  };

  // Reorganize legend into three rows instead of two for better spacing
  const itemsPerRow = Math.ceil(stackedBarData.length / 3); // Split items into three rows
  ...
  stackedBarData.forEach((item, index) => {
    ...
    const rowItems = rowNumber === 2 ? 
                    (stackedBarData.length - (2 * itemsPerRow)) : 
                    itemsPerRow;
    const rowWidth = rowItems * (legendItemWidth + legendSpacing) - legendSpacing;
    const rowStartX = (doc.page.width - rowWidth) / 2;
    
    // Position legend item
    const legendItemX = rowStartX + (columnIndex * (legendItemWidth + legendSpacing));
    const rowY = legendY + (rowNumber * 15); // 15 points spacing between rows
    ...
    doc.rect(legendItemX, rowY, 8, 8)
       .fillColor(colorMapping(item.color))
       .fill();

    // Draw label with percentage - display full text without wrapping
    doc.fillColor('black')
       .fontSize(6)  // Small font size
       .text(`${item.label} - ${percentage}%`,  // Removed (${item.value}) from display
         legendItemX + 12,
         rowY,
         { 
           width: legendItemWidth * 1.5, // Increased width to accommodate longer labels
  ...
  // Add note text below legend with exclamation mark - adjust position for three-row legend
  const noteY = legendY + 65;  // Adjusted for three rows
