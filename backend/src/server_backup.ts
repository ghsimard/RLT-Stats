...
app.get('/api/years-distribution', async (req, res) => {
  try {
    ...
    const data = await getYearsDistribution(decodeURIComponent(school));
...
app.get('/api/feedback-distribution', async (req, res) => {
  try {
    ...
    const data = await getFeedbackDistribution(decodeURIComponent(school));
...
app.get('/api/schedule-distribution-estudiantes', async (req, res) => {
  try {
    ...
    // Define colors for schedules
    const colors = {
      'Mañana': '#4e73df',
      'Tarde': '#1cc88a',
      'Completa': '#36b9cc',
      'Noche': '#f6c23e',
      'Fin de semana': '#e74a3b'
    };
    
    // Map the data
    const scheduleData = result.rows.map((row, index) => ({
      label: row.schedule || 'Sin especificar',
      value: parseInt(row.count),
      color: colors[row.schedule as keyof typeof colors] || `hsl(${index * 30}, 70%, 60%)`
    }));
