...
export const generateSummaryPage = async (doc: CustomPDFKit, school: string): Promise<void> => {
  ...
  const drawStackedBar = (label: string, values: { S: number, A: number, N: number }, y: number) => {
    ...
    // Store original percentages for the legend
    const originalPercentages = { ...normalizedValues };
    ...
    const sWidth = Math.max(normalizedValues.S > 0 ? 5 : 0, (normalizedValues.S / 100) * stackedBarWidth);
    doc.rect(currentX, y, sWidth, barHeight)
       .fillColor('#009900') // Soothing green
       .fill();
    
    // Only show percentage if segment is wide enough
    if (normalizedValues.S >= 8) {
      doc.fillColor('white')
         .fontSize(8)
         .text(`${normalizedValues.S}%`,
           currentX + (sWidth/2) - 8,
           y + (barHeight/2),
           { baseline: 'middle' });
    } else if (normalizedValues.S > 0) {
      // Draw indicator line and external label for small segments
      const centerX = currentX + (sWidth/2);
      
      // Draw diagonal line in the same color as the segment
      doc.save()
         .moveTo(centerX, y + barHeight)
         .lineTo(centerX - 12, y + barHeight + 10) // Increased left offset from -7 to -12
         .strokeColor('#009900') // Same as segment color
         .lineWidth(0.75)
         .stroke()
         .restore();
      
      // Draw text label with green color - centered with diagonal line
      doc.fillColor('#009900') // Soothing green
         .fontSize(7)
         .text(`${normalizedValues.S}%`, 
               centerX - 25, // Increased left offset from -20 to -25
               y + barHeight + 5, // Moved up from +8 to +5
               { width: 14, align: 'center' }); // Reduced width to center text
    }
    currentX += sWidth; // Remove padding after segment

    // Draw A segment (Blue instead of Yellow)
    const aWidth = Math.max(normalizedValues.A > 0 ? 5 : 0, (normalizedValues.A / 100) * stackedBarWidth);
    doc.rect(currentX, y, aWidth, barHeight)
       .fillColor('#4472C4') // Blue instead of yellow
       .fill();
    
    // Only show percentage if segment is wide enough
    if (normalizedValues.A >= 8) {
      doc.fillColor('white') // White text for blue background
         .fontSize(8)
         .text(`${normalizedValues.A}%`,
           currentX + (aWidth/2) - 8,
           y + (barHeight/2),
           { baseline: 'middle' });
    } else if (normalizedValues.A > 0) {
      // Draw indicator line and external label for small segments
      const centerX = currentX + (aWidth/2);
      
      // Draw diagonal line in the same color as the segment
      doc.save()
         .moveTo(centerX, y + barHeight)
         .lineTo(centerX - 12, y + barHeight + 10) // Increased left offset from -7 to -12
         .strokeColor('#4472C4') // Same as segment color
         .lineWidth(0.75)
         .stroke()
         .restore();
      
      // Draw text label with blue - centered with diagonal line
      doc.fillColor('#4472C4') // Blue instead of yellow
         .fontSize(7)
         .text(`${normalizedValues.A}%`, 
               centerX - 25, // Increased left offset from -20 to -25
               y + barHeight + 5, // Moved up from +8 to +5
    ...
    currentX += aWidth; // Remove padding after segment

    // Draw N segment (Red)
    const nWidth = Math.max(normalizedValues.N > 0 ? 5 : 0, (normalizedValues.N / 100) * stackedBarWidth);
    doc.rect(currentX, y, nWidth, barHeight)
       .fillColor('#FF0000') // Standard red
       .fill();
    
    // Only show percentage if segment is wide enough
    if (normalizedValues.N >= 8) {
      doc.fillColor('white')
         .fontSize(8)
         .text(`${normalizedValues.N}%`,
           currentX + (nWidth/2) - 8,
           y + (barHeight/2),
           { baseline: 'middle' });
    } else if (normalizedValues.N > 0) {
      // Draw indicator line and external label for small segments
      const centerX = currentX + (nWidth/2);
      
      // Draw diagonal line in the same color as the segment
      doc.save()
         .moveTo(centerX, y + barHeight)
         .lineTo(centerX - 12, y + barHeight + 10) // Increased left offset from -7 to -12
         .strokeColor('#FF0000') // Same as segment color
         .lineWidth(0.75)
         .stroke()
         .restore();
      
      // Draw text label with red - centered with diagonal line
      doc.fillColor('#FF0000') // Standard red
         .fontSize(7)
         .text(`${normalizedValues.N}%`, 
               centerX - 25, // Increased left offset from -20 to -25
               y + barHeight + 5, // Moved up from +8 to +5
  ...
  drawStackedBar('Prácticas pedagógicas', docentesAvgPracticas, barStartY + barHeight + 15);
  drawStackedBar('Convivencia', docentesAvgConvivencia, barStartY + (barHeight + 15) * 2);

  // Add legend - Moving it directly under the last bar
  const stackedBarLegendY = barStartY + (barHeight + 15) * 3 + 5;
  ...
  const boxSize = 12;
  const textMargin = 16;
  const legendSpacing = 120; // Increased space between legend items from 80 to 120
  
  // S legend with percentage
  ...
  // A legend with percentage
  doc.rect(legendStartX + legendSpacing, stackedBarLegendY, boxSize, boxSize)
     .fillColor('#4472C4') // Blue instead of yellow
     .fill();
  doc.fillColor('black')
     .text(`A veces`, legendStartX + legendSpacing + textMargin, stackedBarLegendY + 2);

  // N legend with percentage
  doc.rect(legendStartX + (legendSpacing * 2), stackedBarLegendY, boxSize, boxSize)
     .fillColor('#FF0000') // Standard red
     .fill();
  doc.fillColor('black')
     .text(`Casi nunca/Nunca`, legendStartX + (legendSpacing * 2) + textMargin, stackedBarLegendY + 2);
  ...
  drawStackedBar('Prácticas pedagógicas', estudiantesAvgPracticas, estudiantesBarStartY + barHeight + 15);
  drawStackedBar('Convivencia', estudiantesAvgConvivencia, estudiantesBarStartY + (barHeight + 15) * 2);

  // Add legend for ESTUDIANTES - Moving it directly under the last bar
  const estudiantesLegendY = estudiantesBarStartY + (barHeight + 15) * 3 + 5;
  ...
  doc.rect(legendStartX + legendSpacing, estudiantesLegendY, boxSize, boxSize)
     .fillColor('#4472C4') // Blue instead of yellow
     .fill();
  doc.fillColor('black')
     .text(`A veces`, legendStartX + legendSpacing + textMargin, estudiantesLegendY + 2);

  // N legend - adjusted to maintain proper spacing after A legend
  doc.rect(legendStartX + (legendSpacing * 2), estudiantesLegendY, boxSize, boxSize)
     .fillColor('#FF0000') // Standard red
     .fill();
  doc.fillColor('black')
     .text(`Casi nunca/Nunca`, legendStartX + (legendSpacing * 2) + textMargin, estudiantesLegendY + 2);
  ...
  drawStackedBar('Prácticas pedagógicas', acudientesAvgPracticas, acudientesBarStartY + barHeight + 15);
  drawStackedBar('Convivencia', acudientesAvgConvivencia, acudientesBarStartY + (barHeight + 15) * 2);

  // Add legend for ACUDIENTES - Moving it directly under the last bar
  const acudientesLegendY = acudientesBarStartY + (barHeight + 15) * 3 + 5;
  ...
  // A legend - moved more to the right
  doc.rect(legendStartX + legendSpacing, acudientesLegendY, boxSize, boxSize)
     .fillColor('#4472C4') // Blue instead of yellow
     .fill();
  doc.fillColor('black')
     .text(`A veces`, legendStartX + legendSpacing + textMargin, acudientesLegendY + 2);

  // N legend - adjusted to maintain proper spacing after A legend
  doc.rect(legendStartX + (legendSpacing * 2), acudientesLegendY, boxSize, boxSize)
     .fillColor('#FF0000') // Standard red
     .fill();
  doc.fillColor('black')
     .text(`Casi nunca/Nunca`, legendStartX + (legendSpacing * 2) + textMargin, acudientesLegendY + 2);
